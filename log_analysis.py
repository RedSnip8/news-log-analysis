...
def get_top_articles():
    ...
    cur = conn.cursor()
    cur.execute("select title, count from articleviewcount order by count desc limit 3;")
...
def get_top_authors():
    conn = psycopg2.connect(dbname=DBNAME)
    cur = conn.cursor()
    cur.execute("select name, sum from authorviewcount order by sum desc limit 3;")
...
def get_high_error_day():
    conn = psycopg2.connect(dbname=DBNAME)
    cur = conn.cursor()
    cur.execute("select day, error_percent from dailystatuslog where error_percent > 1 order by day;")
...
#Get CWD as variable
cwd = os.getcwd()
#Create a new report fiel with time stamp in the log folder under the News directory
report = open(cwd + "/reports/newsReport_" + timestmp + ".txt", "w+")
#write the top three articles, top three authors, list of days with over 1% errors with two new lines between them
...
for day in errors:
     report.write("Day: " + str(day[0]) + "\n Error Percentage: " + str(day[1]) +"%\r\n")
